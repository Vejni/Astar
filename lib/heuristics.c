...
double haversine_dist(node node1, node node2){
    ...
    double a = 2.0 * asin(sqrt(pow(sin(phi_dif * 0.5), 2) + cos(phi2) * cos(phi1) * pow(sin(lambda_dif * 0.5), 2)));
    return R * a;
}

double spherical_law(node node1, node node2){
...
double equirectangular(node node1, node node2){
  double phi1 = node1.lat * PI/180;
  double phi2 = node2.lat * PI/180;
...
double heuristic(node current_node, node goal, int heuristic_func, double heuristic_param){
  double heu_dist;
  if (heuristic_param==0){ //Dijsktra case
    return 0;
  }
